...
def _filter_out_unavailable_players(player_names: List[str], unavailable_player_names: Set[str]) -> List[str]:
    """Remove unavailable players from one bar's player list."""
    # Input: list[player_name] for one bar and set[player_name].
    # Output: filtered list[player_name].
    filtered_player_names: List[str] = []
    for player_name in player_names:
        if player_name not in unavailable_player_names:
            filtered_player_names.append(player_name)

    return filtered_player_names


def _get_wheel_qualifier_filter_sets(rounds: List[Round]) -> Tuple[Set[str], Set[str]]:
    ...
    return qualified_player_names, unavailable_player_names


def _get_wheel_qualifiers_for_bar(player_names_for_bar: List[str], qualified_player_names: Set[str], unavailable_player_names: Set[str]) -> List[str]:
    """Get wheel qualifiers for one bar after filter rules."""
    # Input: list[player_name] for one bar and two set[player_name] filters.
    ...
    return _filter_out_unavailable_players(wheel_qualifier_player_names, unavailable_player_names)
...
def get_wheel_qualifiers_by_bar() -> Dict[str, List[str]]:
    ...
    qualified_player_names, unavailable_player_names = _get_wheel_qualifier_filter_sets(rounds)
    ...
    for bar_name, player_names_for_bar in players_who_played_every_round_by_bar.items():
        wheel_qualifiers_for_bar = _get_wheel_qualifiers_for_bar(player_names_for_bar, qualified_player_names, unavailable_player_names)
