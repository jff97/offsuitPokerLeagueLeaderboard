...
def refresh_rounds_database():
    ...
    historical_round_dates = sorted(set(existing_round_dates) | set(season_history.get_observed_board_wipe_dates(all_rounds)))
    persistence.record_board_wipe_events(
        season_history.get_board_wipe_dates_to_record(all_rounds, stored_round_dates=historical_round_dates)
    )
