...
# Neural Model
# ----------------------------
class DynamicLatentRanker(nn.Module):
    """
    Learns latent player features and interactions from tournament results.
    Embeddings capture skill, consistency, and interaction effects.
    """
    def __init__(self, num_players: int, embed_dim: int = 16, latent_dim: int = 8):
        super().__init__()
        # Learnable embedding per player
        self.embedding = nn.Embedding(num_players, embed_dim)
        # Project embedding to latent features
        self.latent = nn.Linear(embed_dim, latent_dim)
        # Optional non-linear interaction between players in the same round
        self.interaction = nn.Linear(latent_dim, latent_dim)
        # Final scorer producing a single ranking score
    ...
    def forward(self, player_ids: torch.Tensor) -> torch.Tensor:
        ...
        interacted = torch.tanh(self.interaction(latent_feats))
        scores = self.scorer(interacted).squeeze(-1)
        return scores
...
def plackett_luce_loss(scores: torch.Tensor) -> torch.Tensor:
    """
    Listwise ranking loss for a round (Plackett-Luce).
    Encourages correct relative ordering of scores.
    """
...
def train_ranker(
    ...
    """
    Train the latent ranker over all rounds.
    """
...
def build_dynamic_leaderboard(
    ...
    """
    Build a dynamic leaderboard using latent feature discovery.
    """
    ...
    model = DynamicLatentRanker(len(players), embed_dim=embed_dim, latent_dim=latent_dim)
...
def build_filtered_dynamic_leaderboard(
    rounds: List[Round],
    ...
    """
    Build latent leaderboard filtered by TrueSkill uncertainty.
    """
    from offsuit_analyzer.analytics.trueskill_analyzer import build_trueskill_leaderboard

    df_latent = build_dynamic_leaderboard(rounds, epochs, embed_dim, latent_dim, lr, verbose)
