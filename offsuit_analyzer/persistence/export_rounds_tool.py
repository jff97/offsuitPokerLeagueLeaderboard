...
from . import rounds_collection


def _create_zipped_json_rounds_data(rounds_list) -> BytesIO:
    """Convert rounds data to a ZIP file containing formatted JSON.
    
    Args:
        rounds_list: List of Round objects to be converted to JSON and zipped
    
    Returns:
        BytesIO: A memory buffer containing the ZIP file with JSON data
    """
    rounds_data = [round_obj.to_dict() for round_obj in rounds_list]
...
def email_json_rounds_backup() -> None:
    ...
    all_rounds = rounds_collection.get_all_rounds()
    zip_file = _create_zipped_json_rounds_data(all_rounds)
